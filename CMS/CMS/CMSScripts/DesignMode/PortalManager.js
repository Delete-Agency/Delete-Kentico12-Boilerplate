...
function webPartProperties(zoneId, webPartId, nodeAliasPath, instanceGuid, templateId, clientId, variantId, clipBoardItem, key, viewMode) {
...
function zoneProperties(zoneId, nodeAliasPath, templateId, zoneType, layoutZone, clipBoardItem, key, viewMode) {
    this.zoneId = zoneId;
    this.nodeAliasPath = nodeAliasPath;
    this.templateId = templateId;
...
function NewWidget(zone) {
    ...
    setIsLayoutZone(isLayoutZone);
    modalDialog(widgetSelectorUrl + '?aliaspath=' + path + '&culture=' + getCulture() + '&templateid=' + zone.templateId + '&zoneid=' + escape(zone.zoneId) + '&zonetype=' + zone.zoneType + '&layoutzone=' + zone.layoutZone + '&viewMode=' + zone.viewMode, 'selectwidget', '90%', '85%');
}
...
function OnSelectWidget(widgetId, skipDialog) {
    if (skipDialog) {
        ...
        modalDialog(widgetPropertiesUrl + '?aliaspath=' + getAliasPath() + '&culture=' + getCulture() + '&templateid=' + getTemplate() + '&zoneid=' + getZone() + '&zonetype=' + getZoneType() + '&widgetid=' + widgetId + '&layoutzone=' + getIsLayoutZone() + '&isnew=1' + getDashBoardParameter() + '&viewMode=' + getViewMode(), 'configurewidget', 950, '85%');
    }
}
